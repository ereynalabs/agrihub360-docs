...
The node also measures air temperature and barometric pressure. Neither has a catalogue id yet, so neither reaches the Device API.

<Warning>
  The sensor also carries nutrient channels, ids `221` to `223` for nitrogen, phosphorus and potassium. They have no validated accuracy, the app stores them without showing them, and they aren't a supported reading. Don't build on them.
</Warning>
